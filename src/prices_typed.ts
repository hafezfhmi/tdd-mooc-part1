...
function createApp(database: Database) {
  ...
  app.get("/prices", (req, res) => {
    ...
    const date2 = parseDateTemporal(req.query.date as string);
  ...
  function parseDateTemporal(dateString: string | undefined): Temporal.PlainDate | undefined {
    if (dateString) {
  ...
  function calculateReduction(date: Date | undefined, date2: Temporal.PlainDate | undefined) {
    let reduction = 0;
    if (date && date2 && isMonday(date, date2) && !isHoliday(date)) {
      reduction = 35;
    }
    return reduction;
  }

  function isMonday(date: Date, date2: Temporal.PlainDate) {
